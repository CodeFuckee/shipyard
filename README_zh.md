...
- 容器详情：检查配置、实时状态、日志流、环境变量、网络、存储、文件浏览和下载；已映射端口提供“打开网页”操作，以 HTTP 在外部浏览器打开 `http://<Docker 主机>:<宿主机端口>`
...
- 内置 70+ 个预设供应商（参考 cc-switch，含独立 logo，选择后自动填充名称 / Base URL 与默认模型），支持自定义供应商
