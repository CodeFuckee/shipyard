...
- **容器详情**：
  ...
  - **网络 (Network)**：端口映射和网络设置；每个已映射端口均可通过“打开网页”在外部浏览器以 HTTP 打开 `http://<Docker 主机>:<宿主机端口>`。
  - **存储 (Storage)**：卷挂载和绑定挂载。
  - **文件 (Files)**：浏览和下载容器内文件。
...
### 🎨 用户体验
- **AI 助手快捷输入**：点击底部中央 AI 入口即可展开双行输入面板，包含输入区、
  发送按钮与可横向浏览的 Docker 能力展示项；发送后进入完整 AI 对话并实时显示回复。
