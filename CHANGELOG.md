...
- 容器详情的每条已映射宿主机端口新增“打开网页”按钮（issue #45）：从当前 Docker
  服务地址提取主机名，与宿主机端口组合为 `http://<host>:<port>` 后在外部浏览器打开；
  统一使用 HTTP（按需求确认），并兼容 IPv6 地址。未映射、空值、非法或越界端口不会
  生成访问链接；鸿蒙平台复用原生跳转通道。新增 URL 构建、有效端口、IPv6、重复调用及
  详情页按钮显隐测试。


- 容器页面（列表）直接显示容器暴露的端口，无需点击进入详情页即可查看
  （issue #48）：① 后端 `process_container_summary` 将仅暴露未映射的端口
  （`Config.ExposedPorts`）合并进容器摘要——已映射端口保持
  `宿主机端口->容器端口/协议` 格式，未映射的暴露端口以 `端口/协议`（如
  `80/tcp`）补充展示，`ports_list` 同步携带 `public_port: null` 的未映射
  条目，卡片/列表/WebSocket/MCP 各消费方均可直接使用；② 首页容器 tab
  紧凑（list）模式新增端口行（`端口: 8080->80/tcp`），与卡片（grid）模式
  展示口径一致；③ 栈容器页面紧凑模式列表项通过 subtitle 直接展示端口。
  无端口容器不显示端口行（保持原布局）。新增前端 5 个容器列表端口测试
  （list/grid/栈紧凑/栈卡片/无端口边界）与后端 2 个暴露端口合并测试，
  前后端全量测试通过。
